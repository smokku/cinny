...
export function ThreadDrawer({ room, threadRootId, onClose, overlay }: ThreadDrawerProps) {
  ...
  useEffect(() => {
    const markThreadAsRead = async () => {
      ...
      // Send threaded receipt if:
      // - SDK's read marker hasn't reached the last event, OR
      // - server reports unread notifications for this thread
      ...
      const hasServerUnread =
        ...
        room.getThreadUnreadNotificationCount(threadRootId, NotificationCountType.Highlight) > 0;

      if (readUpToId !== lastEventId || hasServerUnread) {
        try {
